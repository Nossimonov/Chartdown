...
6. **Independent review for substantial work.** Before merging a change that touches a *pattern* rather than one site, makes a claim headed for an immutable ADR, or is large enough that your own verification has blind spots, spawn an independent agent — and brief it to hunt **the defect's class**, not to audit the diff. *"Does this problem exist elsewhere?"* presumes the finding and searches for siblings; *"do you see problems with this?"* invites a reviewer to re-derive what you already checked. Every finding worth its cost so far came from the first form, and none was a bug in the change under review. Tell it the diff is assumed correct, name the surfaces to sweep, and require it to **prove its instrument detects a known positive before trusting any null result**. CONTRIBUTING's `Independent review` section carries the worked record.
