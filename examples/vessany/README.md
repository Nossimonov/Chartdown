...
**Status: spec-aligned** — valid under spec v0.7 (sections 01–08). Spec-01 canonicalization (2026-07-20): commas removed, `~90 mi` → `size=90mi`, `gm "…"` → `gm="…"`, multi-word entity references quoted (`to "Merrow's Rest"` — reference form itself is [#11](https://github.com/Nossimonov/Chartdown/issues/11)-pending), and `coastline` moved above the entities that reference it, demonstrating the order-bounded resolution rule. Parser-driven fix (2026-07-20, [#21](https://github.com/Nossimonov/Chartdown/issues/21)): the coastline gained the explicit id `coast` — it was referenced while anonymous, which spec 03 correctly rejects; the parser caught what review missed. Render-review fix (2026-07-20, [#22](https://github.com/Nossimonov/Chartdown/issues/22)): the Vess gained four `via` points — a 700-mile river sketched from two points renders near-straight *by design* (finishing is not inventing, spec 02 §9); sketch density is authorial, and a landmark river deserves the same point budget as a coastline. Fulfills [#5](https://github.com/Nossimonov/Chartdown/issues/5), authored against Elena's region-map narrative in [docs/use-cases.md](../../docs/use-cases.md) §3. This is the acknowledged hard problem: continuous positioning in readable text.
