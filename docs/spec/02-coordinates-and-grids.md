...
| `from <endpoint> [via <controls>] to <endpoint>` — paths; a control is a point, or a **cell** on a grid (#258) | `river "The Vess" : from "The Serpent's Spine" at (720,240) to "Gull Bay"` |
| `from <endpoint> [via <controls>] join <ref>` — confluences | `river bruinen : from misty at (665,320) via (628,440) join mitheithel` |
...
- **A `via` control is a point, or a cell on a grid** *([#258](https://github.com/Nossimonov/Chartdown/issues/258))*. A grid map writes every other position as `D8`, and had no way to shape a course but a world-unit pair — in units that document never uses. `stream : from fountain via D8 to sink-hole` bends the course through that cell, and renders identically to the `path` spelling through the same three cells. A **gridless point on a grid map is reported, not rounded**: cell size follows `scale:`, so the same `(60,40)` would name a different square after a `scale:` edit, and the number in the document would stop being the shape on the map. The reverse holds too — a cell in a `via` on a gridless map names nothing and is reported.
