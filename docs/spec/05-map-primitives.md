...
- **On a DETACHED feature, `reach=` is elongation.** `size=` is the long axis and `reach=` the short one as a multiple of it — the same "the other dimension" it means for juts and bites. `reach=1`, and an absent `reach=`, are a circle. The long axis lies **along the host's local course**, inferred rather than declared, because a long island in a sound parallels the shore it sits off — the same ice cut both. Without this every island was a circle at its longest dimension, which is the decorative state `reach=` exists to end (#159).
