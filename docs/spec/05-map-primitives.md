...
- **`reach=` gives a word its proportions.** `size=` is the extent ALONG the host; `reach=` is how far it carries ACROSS, as a multiple of that size. It is a vocabulary facet, so derivation carries it and an author may override it per entity. Without it a word is decorative: `cove`, `sound` and `fjord` at the same size drew an identical shape and differed only in colour — useless on a coast whose whole character is that one inlet is long and narrow while another is a shallow scoop. The shipped values run from `cove` (0.3) through `bay` (0.5) and `cape` (0.55) to `sound` (1.4) and `fjord` (2).
