...
- **`taper=` gives it a profile.** A raised cosine is always a wedge — widest at the mouth, narrowing to a point — which is right for a cove and wrong for everything glacial. `taper=` is the fraction of the mouth spent narrowing: at `1` the shape is that wedge, and below it the middle is displaced flat so the result is a trench with rounded corners. Hood Canal, Case Inlet and Carr Inlet are near parallel-sided, and without this the only way to reach their depth was a hairline needle that read as a spine on the coast (#158).
