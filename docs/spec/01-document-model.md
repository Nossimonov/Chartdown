...
2. **Header** — every line between the title and the first section. Header lines are ordinary lines (§4) whose subjects are reserved keys. **The first header line is `map:` or `kind:`**, and a document declares one or the other, never both.
   ...
   **A header key whose value the language constrains — whether to a set or to a format — rejects anything outside it, as an error.** For formats (`extent:`, `seed:`, `scale:`, `chartdown:`) the reason is forward compatibility as much as correctness: each is read through a coercion with a fallback, so a malformed value silently yields a document built on a default its author never wrote. Such a document breaks the day the assumed spelling becomes legal — were `seed:` ever to accept a word by hashing it, every map that had quietly rendered at seed `0` would change its geometry at once, having never been told it wrote something the language did not understand. Rejecting now is what keeps those doors open.

   **A header key whose values the language enumerates is a closed set, and a value outside it is an error** — `map:`, `kind:`, `labels:` (spec 07 §3), and the `on`/`off` furniture toggles (spec 07 §4). Unlike a vocabulary facet, an out-of-set value here cannot be an author extending the language, because the language defines every legal value; it can only be a mistake. Failing loud matters more than usual for these keys: each one selects behaviour by exact match, so a near-miss an author would reasonably write — `legend: yes`, `compass: true`, `numbers: ON` — silently selects the *default* and turns the requested feature off, leaving a document that says one thing and renders another.
