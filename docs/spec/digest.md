...
- **Placed morphology** (05 §4, #93, ADR 0023): cape headland peninsula spit (`morph=jut`) · bay cove sound fjord (`morph=bite`) · island islet atoll oxbow (`morph=detached`). Discrete features ON a line, each a REAL addressable entity — `cape : on coast at (150,182) size=22mi`, `island himling "Himling" : near coast at (95,250) size=8mi`. NO NEW SYNTAX: uses 02 §7's existing `on <ref> at <point>` / `near <ref>` forms. `reach=` gives a word its PROPORTIONS — `size=` is extent ALONG the host, `reach=` how far it carries ACROSS as a multiple of it (calibrated vs Puget Sound #161: cape 0.55 · bay 1 · peninsula 1.6 · cove 3 · spit 5 · sound 6 · fjord 20) · on a DETACHED feature `reach=` is ELONGATION instead (size = long axis, reach = short as a multiple; 1 or absent = a circle) and the long axis follows the HOST's local course, inferred not declared — a long island parallels its shore (#159) · `taper=` is the fraction of the mouth spent NARROWING (1 = a wedge, lower = a parallel-sided trench — a fjord is a drowned valley, and at wedge profile the only way to its depth was a needle, #158) · direction comes from the water and an ENCLOSED sea declares it by SHAPE: a bounded body given as an area following its coasts settles the side, since a half-plane can only describe an open ocean (#157); a vocabulary facet, so derivation carries it and an entity may override. Without it the words were decorative — cove/sound/fjord drew the SAME shape at the same size. `morph=` is a CLOSED set saying what the geometry does to the host; the WORD says what the thing is and is what a theme colours — independent on purpose, so `bay` bites while being water and `oxbow` detaches while being water. Direction is INFERRED from which side the water lies on, never declared. Geometry is a pure function of (kind, anchor, size), independent of `seed:` and of every other entity. PROMOTION IS GEOMETRY-STABLE: adding an id/name MUST NOT move the shape. A feature is drawn at its DECLARED size or reported — never quietly resized: clamping is non-conforming because it makes `size=` a lie (the same 90mi cape would differ by stretch). Cannot-draw = ERROR naming feature, size and host, course left undisturbed. The course may not cross itself AND may not fold to a cusp (simplicity alone is insufficient — a fold draws a spike without intersecting). The displacement is ONE direction (the host normal at the anchor), never a per-point offset: per-point normals pinch on a headland and make drawable bays undrawable. 02 §9's noise-free spline is AFFIRMED, not revised. Line-BRANCHING (`delta`, `fork`) is staged and deliberately NOT in the stdlib yet, so it gets unknown-word treatment rather than a declaration that parses clean and draws nothing
