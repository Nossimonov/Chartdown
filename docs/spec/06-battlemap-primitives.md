...
- **A stair is a way in** *([#301](https://github.com/Nossimonov/Chartdown/issues/301), [ADR 0052](../decisions/0052-a-stair-is-a-way-in.md))*: a `stairs` or `ramp` feature standing inside a footprint is an entrance to it, **whether or not it carries `to=`**, and so is any word deriving from either (spec 04 §2). §8's `to=` is what connects one declared level to another; reaching a room is a separate question, and on a **single-level** document there is no level for a `to=` to name. Without this, the only ways to say that a cellar is entered by its stair were to invent a door the map does not have or to invent a second level for the stair to point at — both of which change the map to satisfy a check. `slope` is deliberately excluded: it is a graded surface *within* a level (`slope : terrain`), and walking up one does not arrive from a storey the document never drew.
...
- **Ledges are emergent, not drawn**: wherever adjacent placements' elevations differ, the renderer draws a theme-styled edge, and the drop is the difference — precisely the number the table asks for. There is no cliff-tracing grammar. *([#425](https://github.com/Nossimonov/Chartdown/issues/425))* The terms that rule depends on:
  - **A cell's elevation is the winning declaration on it** — the last placement to cover it, exactly as §3 resolves solidity (*“always the winning declaration on a cell, never merely one that was made”*). The blanket-ground idiom below lays the level first and paints over it, so the raised thing is declared later and wins.
  - **Ground nobody declared is elevation `0`**, including beyond the edge of the grid. So a terrace raised over open ground is bounded by a ledge all the way round without the author declaring the plain about it — which is what “no cliff-tracing grammar” has to mean.
  - **Adjacent means sharing a cell EDGE**, not a corner; a ledge is a line along a boundary, and two cells meeting at a point have none.
  - **The higher side owns the edge**, and its ticks fall outward, away from the ground that ends there — the same mark `drop` makes.
  - **A cell edge carries at most ONE cliff.** `drop` and an elevation difference are two reasons for the same mark, not two marks: an area that is both raised and flagged draws its boundary once.
...
- **A crossing declares which band is over, and the PLACEMENT makes it a crossing** *([#398](https://github.com/Nossimonov/Chartdown/issues/398), [ADR 0053](../decisions/0053-a-crossing-declares-which-band-is-over.md))*. Two bare `on` references name the intersection of two bands for **any** feature word — the form resolves as the table above says, with no condition on vocabulary, so an unknown word is placed rather than refused (spec 04 §3). Which band is drawn on top is the closed facet **`over=`**, values `path` and `water`, **inherited through derivation** (spec 04 §2): `ford : feature states=difficult over=water` and `bridge : feature over=path` are the standard library declaring the hint those words were invented to carry, so `culvert : ford` and `causeway : feature over=path` both work and neither has to claim to be something it is not. An entity **carrying** `over=`, declared or inherited, is a crossing however it is placed — which is what makes `ford : E5` a crossing with no `on` references at all. Absent any declaration the default is **`over=path`**, applied silently: the placement has already said a crossing is meant, and a wrong guess is visible on the page and one word to correct. `over=` decides **stacking only** — colour comes from the word's theme entry, so a setting can style its causeways without touching how they stack.
...
- **Implied-crossing warning**: a water-path × road overlap not covered by any crossing's cells produces a renderer warning naming both entities and the cell — the render would otherwise imply a crossing nobody declared. What silences it is a declared crossing at that cell, whatever it is called.
...
- **Connectors**: *any feature carrying `to=<level>`* connects levels — for **connection** `stairs` and `ramp` carry no privilege, and `ladder : stairs` (or any word at all) works identically. (They are load-bearing for a different question: §3 makes a stair standing in a room a way *into* that room, `to=` or no `to=`. Connecting two levels and being an entrance are separate facts, and only the first is what `to=` states.) Landings default to the same cell; `at=<cell>` places a differing landing. The destination panel shows the reciprocal landing automatically unless an explicit connector is declared at that cell. **The suppression test is on the declaration** *([#319](https://github.com/Nossimonov/Chartdown/issues/319), [ADR 0046](../decisions/0046-a-landing-is-suppressed-by-a-declaration-not-by-a-drawing.md))*: a connector the document states at that cell suppresses the far end's projection whether or not the current mode draws it. The projection's **source** is the drawn model — a `hidden` connector projects no landing anywhere (spec 01 §6) — so the two halves of this rule read **different** entity sets, and a renderer that tests the drawn set on **both** sides is **non-conforming**. Each panel is therefore governed by the end declared *on it*, which is what lets a double declaration spell a one-sided secret: `hidden` on the house end and plain on the cellar end is a trapdoor secret from above and obvious from below. Testing the drawn set instead did the opposite of what `hidden` asks — player mode stripped the secret connector and the far end's projection immediately refilled the empty cell, drawing the secret onto the players' sheet. Connectors expose the **reserved auto-states `up` and `down`** (derived from level order) to themes — `ladder.up : glyph=…` — through the ordinary spec 08 machinery. A `to=` naming an undeclared level fails loud.
- **Shafts span more than two levels** *(from proposal [#112](https://github.com/Nossimonov/Chartdown/issues/112))*. A connector between two adjacent panels cannot describe the vertical things a deep dungeon is made of, so two spellings extend it:
  - **`to=<level>..<level>`** declares landings on **every** level in the range, from one line. A stair from the Seventh Level to the Gates is one stair; writing it as four connectors gave it four ids, four chances to typo a column, and nothing anywhere saying they were the same flight. The annotation on each panel names the **next landing in the direction of travel**, not the far end — standing mid-flight, the step about to be taken is what matters. **The annotation is a property of the flight, not of the panel that drew it** *([#321](https://github.com/Nossimonov/Chartdown/issues/321), [ADR 0048](../decisions/0048-an-annotation-names-the-nearest-landing-of-its-own-flight.md))*: a flight's landings are every level its `to=` names, **plus the level it is declared on**, **minus every level its `through=` names — a `through=` level is never named as a landing**, since it has none. Each panel names whichever landing is **nearest** it in `levels:` order; where a panel is equidistant from a landing above and one below, it names the **one above**. This governs the **reciprocal** panels too — a renderer that annotates a projected landing with the level the connector was *declared on*, rather than with the flight's next landing from that panel, is **non-conforming**, and so is one that prints the `to=` value verbatim where that value is a range (`▼ upper..cellar` names no level at all).
...
- **`line_of_sight`** — structure perimeter walls (per cell edge, minus `ruined` sides, coincident edges deduplicated per §3) minus **every opening edge**, plus **freestanding barriers that stop sight**. Which those are is decided by the **facet, not the word** — a barrier occludes unless its resolved `sight=` is `all`, and a barrier that says nothing about sight is opaque (the opposite of an opening's default). This is §3's rule, applied to the same declaration wherever it is written: `hedge : barrier sight=all` is a hole in `line_of_sight` standing on its own exactly as it is replacing a room's side, and `thicket : fence sight=none` occludes despite its ancestry. Naming `fence` here instead exported both of those backwards *([#396](https://github.com/Nossimonov/Chartdown/issues/396))*. Portals own their edges’ occlusion.
...
- **`lights`** — every entity carrying an emitter parameter for a field that **derives from `light`**, positioned at its placement’s center, range converted to grid units via `scale:`. The parameter is resolved through the vocabulary, not the literal key (spec 04 §5, [ADR 0016](../decisions/0016-derivation-carries-word-keyed-behaviour.md)), so `glow : light` with `glow=20ft` exports exactly as `light=20ft` does. Fields that are **not** light do not: a setting’s `radiation : field` renders as ink like any field, but UVTT `lights` and `ambient_light` mean illumination, and a radiation source is not a lamp. That is an **interop boundary, not a second class of field** — the same rule governs `ambient_light` above *([#395](https://github.com/Nossimonov/Chartdown/issues/395))*.
...
| `unreachable-room` | A footprint has neither an opening on its perimeter, nor a connector inside it, nor a **stair or ramp** inside it (§3) | A room whose only way in is a stair **declared on another level**; §8's reciprocal landing is what puts it here. A stair standing **in** the room is a way in with no `to=` at all, which is the only thing a single-level map can write |
...
Four rules run beneath the table and decide most of the readings in its right-hand column:
...
- **A redaction is not the document** *([#320](https://github.com/Nossimonov/Chartdown/issues/320), [ADR 0045](../decisions/0045-a-redaction-is-not-the-document.md))*. `hidden` and `[gm]` decide what a **reader** is shown (spec 01 §6); they do not change what the document **says**. These checks therefore read the **declared** model, and a document MUST produce the same lints in `gm` and in `player` mode — a renderer whose lint set depends on the render mode is **non-conforming**. Linting the redaction instead reported *every* secret entrance as `unreachable-room`, since stripping the only way in leaves a room with no way in: a warning that described the redaction accurately and the document not at all. It was unsilenceable by §10's own no-suppression rule, it arrived by default because `render` defaults to `player`, and the single edit that cleared it — give the room a real opening — was the edit that destroyed the secret. The redaction is still what is **drawn**; this governs only what is **reasoned about**. **This holds for every diagnostic a document can provoke, not only §10’s six** *([#397](https://github.com/Nossimonov/Chartdown/issues/397))*: §6’s implied-crossing warning read the drawn model, so a `hidden` ford reported that the road crossed the river with no crossing — a crossing the author had declared on the line above. Whatever reasons about a document reads the document.
